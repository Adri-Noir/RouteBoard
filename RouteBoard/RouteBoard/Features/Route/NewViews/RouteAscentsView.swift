...
struct RouteAscentsView: View {
  ...
  @State private var showAllAscentsSheet = false

  private let previewAscentsCount = 1

  private var previewAscents: [Components.Schemas.AscentDto] {
    guard let ascents = route?.ascents, !ascents.isEmpty else { return [] }
    return Array(ascents.prefix(previewAscentsCount))
  }

  private var totalAscentsCount: Int {
    return route?.ascents?.count ?? 0
  }
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 10) {
      ...
      if previewAscents.isEmpty {
        ...
        VStack(spacing: 15) {
          ForEach(previewAscents, id: \.id) { ascent in
            RouteAscentRowView(ascent: ascent)
          }

          if totalAscentsCount > previewAscentsCount {
            Button(action: {
              showAllAscentsSheet = true
            }) {
              HStack {
                Text("View All \(totalAscentsCount) Ascents")
                  .fontWeight(.semibold)
              ...
              .background(
                RoundedRectangle(cornerRadius: 20)
                  .stroke(Color.newPrimaryColor, lineWidth: 1.5)
    ...
    .sheet(isPresented: $showAllAscentsSheet) {
