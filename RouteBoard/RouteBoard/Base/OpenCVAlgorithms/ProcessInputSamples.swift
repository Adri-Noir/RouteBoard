...
class DetectSample {
    ...
    init(route: UIImage, path: UIImage, routeId: Int) {
        self.route = route
        self.path = path
...
class DetectProcessedSample {
    ...
    init(referenceKP: [opencv2.KeyPoint], referenceDES: opencv2.Mat, routeReference: UIImage, routeId: Int) {
...
class DetectProcessedFrame {
    ...
    init(frame: UIImage, routeId: Int) {
        self.frame = frame
        self.routeId = routeId
    }
}
...
class ProcessInputSamples {
    ...
    func processInputSamples(samples: DetectInputSamples) {
        ...
        for sample in samples.samples {
            ...
            let resizeFactor = Double(MAX_RESOLUTION_PX) / Double(max(routeImageMatrix.rows(), routeImageMatrix.cols()))
            
            
            Imgproc.resize(src: routeImageMatrix, dst: resizedRouteImageMatrix, dsize: Size(), fx: resizeFactor, fy: resizeFactor)
            Imgproc.resize(src: pathImageMatrix, dst: resizedPathImageMatrix, dsize: Size(), fx: resizeFactor, fy: resizeFactor)
            
            
            var routeKeypoints: [opencv2.KeyPoint] = []
            let routeDescriptors = Mat()
            
            sift.detectAndCompute(image: resizedRouteImageMatrix, mask: Mat(), keypoints: &routeKeypoints, descriptors: routeDescriptors)
            ...
            processedSamples.addSample(sample: DetectProcessedSample(referenceKP: routeKeypoints, referenceDES: routeDescriptors, routeReference: sample.path, routeId: sample.routeId))
        }
    }
    ...
    func detectRoutesAndAddOverlay(inputFrame: UIImage) -> DetectProcessedFrame {
        ...
        Imgproc.resize(src: frameMatrix, dst: resizedFrameMatrix, dsize: Size(), fx: DROP_INPUTFRAME_FACTOR, fy: DROP_INPUTFRAME_FACTOR)
        
        let frameOutput = Mat.zeros(resizedFrameMatrix.rows(), cols: resizedFrameMatrix.cols(), type: resizedFrameMatrix.type())
        
        var frameKeypoints: [opencv2.KeyPoint] = []
        let frameDescriptors = Mat()
        
        sift.detectAndCompute(image: resizedFrameMatrix, mask: Mat(), keypoints: &frameKeypoints, descriptors: frameDescriptors)
        ...
        if frameKeypoints.count < 3 || frameDescriptors.empty() {
            return DetectProcessedFrame(frame: frameOutput.toUIImage(), routeId: -1)
        }
        
        var minDistance = Double.greatestFiniteMagnitude
        var closestRouteId = -1
        ...
        for sample in processedSamples.samples {
            ...
            matcher.knnMatch(queryDescriptors: sample.referenceDES, trainDescriptors: frameDescriptors, matches: &knnMatches, k: 2)
            ...
            if numberOfGoodPoints > MIN_MATCH_COUNT {
                ...
                let homography = Calib3d.findHomography(srcPoints: srcMat, dstPoints: dstMat, method: Calib3d.RANSAC, ransacReprojThreshold: 5.0)
                
                if !homography.empty(), homography.rows() == 3, homography.cols() == 3 {
                    let transformedCenter = MatOfPoint2f(array: [frameCenter])
                    let transformedResult = MatOfPoint2f()
                    
                    Core.perspectiveTransform(src: transformedCenter, dst: transformedResult, m: homography.inv())
                    
                    let routeMatrix = Mat(uiImage: sample.routeReference)
                    let routeReferencePoint = opencv2.Point2f(x: Float(routeMatrix.cols()) / 2.0, y: Float(routeMatrix.rows()) / 2.0)
                    let distance = Core.norm(src1: transformedResult, src2: MatOfPoint2f(array: [routeReferencePoint]))
                    
                    let overlay = Mat()
                    Imgproc.warpPerspective(src: routeMatrix, dst: overlay, M: homography, dsize: Size(width: frameCols, height: frameRows))
        ...
        Imgproc.resize(src: frameOutput, dst: frameOutput, dsize: Size(width: frameMatrix.cols(), height: frameMatrix.rows()))
        Core.addWeighted(src1: frameMatrix, alpha: 1.0, src2: frameOutput, beta: 1.0, gamma: 0.0, dst: frameOutput)
